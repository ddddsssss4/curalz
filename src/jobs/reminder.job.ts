...
const checkReminders = cron.schedule("* * * * *", async () => {
  ...
  try {
    // Mark any past events that are still pending as already sent
    await Event.updateMany(
      { datetime: { $lt: now }, reminderStatus: "pending" },
    ...
    for (const event of upcomingEvents) {
      ...
      if (event.reminderOffsets && event.reminderOffsets.length > 0) {
        for (const offset of event.reminderOffsets) {
          if (Math.abs(minutesUntil - offset) <= 1) {
            // 1 minute tolerance
            console.log(
              `[REMINDER] Sending reminder for event "${event.title}" to ${event.userId}`,
            );
